...
public class DirectionsControl implements IDirectionsControl {
    ...
    private static final String SQL_SELECT_POINT_DIRECTIONS = "SELECT dir.id, dir.direction FROM directions as dir join map_points as mp on dir.id=mp.direction where mp.coordinates=st_geomfromtext(?)";
    ...
    @Override
    public Direction[] getPointDirections(String coordinates) throws DAOException {

        try {

            return (Direction[]) RequestExecutor.getSignsStaffWithDifferentParameters(SQL_SELECT_POINT_DIRECTIONS, new Direction(), coordinates);
